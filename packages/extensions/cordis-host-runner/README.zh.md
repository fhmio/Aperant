...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-cordis-host-runner)是每个受支持字段的穷尽式真源。
...
- [生成的配置目录](../../../docs/config-catalog.md#deepseek-aidsh-cordis-host-runner)——每个受支持配置字段。
- [extensions 子系统](../../../docs/subsystems/extensions.md)——生成的 `ctx.cordisInspect` 与 `ctx.dynamicCordisRunner` API 及 `cordis/*` 事件。
