...
import { ar, en, type CommandKey } from '../src/client/locales.ts'
...
describe('candidates', () => {
  ...
  describe('menu presentation (design doc for #3567)', () => {
    ...
    it('a same-name override keeps its own presentation even when it copies the first-party description', async () => {
      ...
      expect(source.matchSpace!(proj('s1'), '/unknown')).toBeUndefined()
      expect(await source.matchEnter!(proj('s1'), '/unknown x', new AbortController().signal, { attachments: 0 })).toBeUndefined()
