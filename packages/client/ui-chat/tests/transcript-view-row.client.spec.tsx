...
import { ar, en } from '../src/client/locale.ts'
...
describe('TranscriptViewRow', () => {
  ...
  it('shows the conversation-display values in Arabic', () => {
    mount('compact', ar)
    fireEvent.click(screen.getByRole('button', { name: ar['settings.transcript.compact'] }))
    fireEvent.click(screen.getByRole('menuitem', { name: ar['settings.transcript.normal'] }))
    expect(screen.getByRole('button', { name: ar['settings.transcript.normal'] })).toBeDefined()
  })
})
