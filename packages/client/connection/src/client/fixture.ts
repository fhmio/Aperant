...
const FIXTURE_SYSTEM_PROMPT = 'You are the Aperant fixture assistant. Answer concisely and invoke tools when needed.'
...
function buildAlphaLog(): SessionEvent[] {
  ...
  for (let turn = 0; turn < 60; turn++) {
    ...
    const userSeq = push({
      ...
      data: userMessage(text(turn === 59 ? USER_MARKDOWN_LITERAL : `Question ${turn}: fixture history message for pagination and rendering verification.`)),
    })
    if (turn === 0) {
      push({
        type: 'session/title',
    ...
    blocks.push({ type: 'text', text: turn === 59 ? MARKDOWN_FIXTURE : `Answer ${turn}: fixture historical reply text.` })
    if (withTool) {
  ...
  push({ type: 'user/message', surfaceOp: 'append', data: userMessage(text('Question 72: Please list all 100 items completely.')) })
  push({ type: 'step/start', data: { turn: 72, step: 0 } })
  push({
    type: 'assistant/message',
    surfaceOp: 'append',
    data: { turn: 72, step: 0, message: assistantMessage(text('Item 1: First. Item 2: Second. Item 3: Cut off halfway')) },
  })
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixtureQuestions: readonly FixtureQuestionItem[] = [
    {
      id: 'harness-profile',
      header: 'Preferences',
      question: 'Which candidate profile do you prefer to hire?',
      options: [
        { label: 'Engineering-focused (Recommended)', description: 'Focuses on runtime, tool execution, sandboxing, tracing, and debugging.' },
        { label: 'Research-oriented', description: 'Focuses on agent cognition, evaluation methodology, and long-term potential.' },
        { label: 'Balanced', description: 'Balanced engineering and agent architecture skills.' },
      ],
    },
    {
      id: 'work-mode',
      ...
      options: [
        { label: 'Prototype first (Recommended)', description: 'Validate key assumptions quickly with working code.' },
        { label: 'Design first', description: 'Establish protocols and risks before implementation.' },
      ],
    },
    {
      id: 'signals',
      ...
      detail: 'Select according to hiring goals; skip if no preference.',
      multiSelect: true,
      options: [
        { label: 'System Design' },
        { label: 'Code Quality' },
  ...
  const commandRemotes = {
    list(id: SessionId): RpcResult<readonly CommandDescriptor[]> {
      ...
      return {
        ok: true,
        value: [
          { name: 'compact', description: 'fixture: compact session context' },
    ...
    execute(id: SessionId, line: string, attachments: readonly unknown[] = []): RpcResult<CommandExecution | undefined> {
      ...
      const outcomes: Record<string, string> = {
        compact: 'fixture: compacted (stub)',
        echo: args.trim(),
        plan: args.trim() === 'off'
  ...
  const workspaceFileLines = (path: string): string[] => {
    const name = path.slice(path.lastIndexOf('/') + 1)
    const head = [`# ${name}`, '', 'Sample text in fixture mode for sidebar text preview verification.', 'Production builds read file from workspace.']
    return path.includes('demo') || path.includes('huge')
      ? [...head, ...Array.from({ length: 12_000 }, (_, index) => `Line ${index + 5}: Long sample text for pagination and scroll verification.`)]
      : head
  }
  ...
  const timingHooks = {
    ...
    beginModelRetry(id: string): void {
      ...
      pushAssistant(sessionId, { type: 'text-delta', index: 0, text: 'Partial reply to be rolled back' })
    },
    ...
    scheduleModelRetry(id: string, retry = 1, delayMs = 450): void {
      ...
      if (!scenario.stepStarted) {
        ...
        pushAssistant(sessionId, { type: 'block-start', index: 0, blockType: 'text' })
        pushAssistant(sessionId, { type: 'text-delta', index: 0, text: `Attempt ${String(retry)} reply to be rolled back` })
    ...
    completeModelRetry(id: string): void {
      ...
      if (scenario === undefined) throw new Error(`fixture: no model retry scenario for ${id}`)
      retryScenarios.delete(sessionId)
      const completed = 'Completed reply after retry'
      beginAssistant(sessionId, scenario.turn, 1)
      pushAssistant(sessionId, { type: 'block-start', index: 0, blockType: 'text' })
          ...
          : userText === 'report model'
            ? (() => {
              ...
              return `Current model: ${selection?.provider ?? 'unknown'}/${selection?.model ?? 'unknown'}`
                  + (selection?.reasoningEffort === undefined ? '' : ` · Reasoning: ${selection.reasoningEffort}`)
            })()
            : `Echo: ${userText}. This is a fixture streaming response for typewriter growth and settlement verification.`,
      )
      return sessionOk({ accepted: true as const })
    },
    attachment: (request) => {
  ...
  const approvalInvocation = (): FixtureRemoteEventInvocationFrame => ({
    ...
    request: {
      ...
      reason: 'fixture persistent approval (actionable: approve/reject to dismiss)',
    },
  })
    ...
    call(channel, endpoint, payload, signal) {
      ...
      switch (endpoint) {
        ...
        case 'skills/list': {
          ...
          return sessionOk({
            skills: [
              { name: 'fixture-demo', description: 'fixture skill sample', whenToUse: 'UI catalog rendering verification only', modelInvocable: true },
