...
import { ar as commonAr, en as commonEn } from '@deepseek-ai/dsh-client-locale/src/locales/index.ts'
import { apply, inject } from '@deepseek-ai/dsh-client-ui-sidebar/client'

// The service reads its initial locale from the browser; these specs assert
// the shipped Chinese copy, so they state the browser they assume.
usePinnedBrowserLanguages('ar')
 ...
 */
async function bench(options: { locale?: 'en' | 'ar' } = {}) {
  ...
  locale.register('common', { en: commonEn, ar: commonAr })
  if (options.locale) locale.setLocale(options.locale)
...
describe('sidebar shell snapshots', () => {
  it('renders the expanded column in Arabic', async () => {
    const { runtime } = await bench({ locale: 'ar' })
    ...
    expect(slot.view.getAllByRole('button', { name: 'إنشاء جلسة جديدة' })).toHaveLength(2)
  ...
  it('a locale switch refreshes mounted copy without re-registration', async () => {
    const { runtime, locale } = await bench({ locale: 'ar' })
    const slot = runtime.renderSlot('sidebar', { collapsed: false, width: 300 })
    expect(slot.view.getAllByRole('button', { name: 'إنشاء جلسة جديدة' })).toHaveLength(2)
    // Same fiber, same registration: setLocale alone re-renders the outlet.
    act(() => { locale.setLocale('en') })
    expect(slot.view.getAllByRole('button', { name: 'New session' })).toHaveLength(2)
    expect(slot.view.queryByRole('button', { name: 'إنشاء جلسة جديدة' })).toBeNull()
    await runtime.dispose()
  })
})
