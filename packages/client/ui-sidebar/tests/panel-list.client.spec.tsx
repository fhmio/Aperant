...
async function bench(collapsed = false) {
  ...
  await runtime.mount({
    ...
    apply(ctx: Context) {
      ...
      ctx.effect(() => locale.register('common', { en: commonEn, ar: commonAr }), 'panel test: common locale')
      ctx.effect(() => locale.register('sidebar-panel-test', {
        ar: { alpha: 'لوحة ألفا' }, en: { alpha: 'Alpha panel' },
...
describe('sidebar global panels', () => {
  ...
  it('refreshes locale labels without re-registering icons or changing plain labels', async () => {
    ...
    await waitFor(() => {
      expect(within(navigation).getByRole('button', { name: 'لوحة ألفا' }).textContent).toBe('لوحة ألفا')
    })
