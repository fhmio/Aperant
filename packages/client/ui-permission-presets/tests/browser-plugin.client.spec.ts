...
import { accessAr, accessEn } from '../src/client/locales.ts'
...
describe('ui-permission browser plugin', () => {
  ...
  it('availability follows the projection key; options mark the current value active and exclude custom', async () => {
    ...
    expect(localized.map(option => option.label)).toEqual(['للقراءة فقط', 'كتابة في مساحة العمل', 'وصول كامل'])
    expect(localized.find(option => option.id === 'danger-full-access')?.confirmation).toEqual({
      title: 'هل تريد تفعيل الوصول الكامل؟',
      description: accessAr['confirm.description'],
      acknowledgeLabel: 'أدرك المخاطر وأرغب في المتابعة',
      cancelLabel: 'إلغاء',
