...
describe('LocaleRuntime', () => {
  ...
  it('translates through the active-locale -> en -> key chain', () => {
    const { svc } = make()
    svc.register('ns', 'ar', { hello: 'مرحبا' })
  ...
  it('interpolates {name} params and leaves unknown placeholders intact', () => {
    const { svc } = make()
    svc.register('ns', 'ar', { greet: 'مرحبا، {name}！المرة {n}', partial: '{known} و {unknown}' })
    const t = svc.bind('ns')
    expect(t('greet', { name: 'العالم', n: 2 })).toBe('مرحبا، العالم！المرة 2')
  ...
  it('registers an external locale for selection, translation, persistence, and reversible disposal', () => {
    ...
    dispose()
    expect(svc.getLocale().active).toBe('ar')
    expect(svc.getLocale().locales.map(locale => locale.id)).toEqual(['en', 'ar'])
  ...
  it('rejects a fallback cycle exposed by re-registering an unloaded language', () => {
    ...
    expect(svc.getLocale().locales.map(locale => locale.id)).toEqual(['en', 'ar', 'fr-CA'])
  })
  ...
  it('serves English as both the opening locale and the dictionary fallback', () => {
    ...
    vi.stubGlobal('window', undefined)
    const { svc } = make()
    // A key present only in en resolves for an ar reader through the fallback.
    svc.register('ns', 'ar', {})
    svc.register('ns', 'en', { onlyEn: 'English only' })
    svc.setLocale('ar')
    expect(svc.getLocale().active).toBe('ar')
    expect(svc.bind('ns')('onlyEn')).toBe('English only')
    // The reverse no longer resolves: an ar-only key is unreachable from en, so
    // the key itself surfaces (fail loud) rather than silently rendering ar.
    svc.register('ns2', 'ar', { onlyAr: 'فقط عربي' })
    svc.register('ns2', 'en', {})
    svc.setLocale('en')
