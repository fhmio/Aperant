...
import { ar, en } from '@deepseek-ai/dsh-client-ui-conversation/src/client/locales.ts'
...
const arT: GenericToolCardProps['t'] = makeTranslate(ar, commonAr)
...
describe('terminalCardModel', () => {
  ...
  it('keeps terminal_send copy semantic until the render locale is known', () => {
    ...
    expect(localizeTerminalCardModel(model, arT)).toMatchObject({
      description: 'الطرفية pty-3', card: { command: '(إرسال مدخلات)' },
    })
  })
...
describe('chat row terminal body', () => {
  ...
  it.each([
    { locale: 'ar', translate: arT, description: 'الطرفية pty-3', command: '(إرسال مدخلات)' },
