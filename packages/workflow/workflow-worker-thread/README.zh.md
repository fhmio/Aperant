...
负责该引擎的消费方可以为一次运行设置 `WorkflowStartRequest.subagentProvider` 与 `WorkflowStartRequest.maxTotalAgents`——这是引擎级策略，不是脚本钩子；普通 `workflow` 工具两者都不设置，单次运行的子 agent 总数上限可以降低、但绝不能提高已配置的上限。生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-workflow-worker-thread)是每个受支持字段的穷尽式真源。
...
- [工作流子系统](../../../docs/subsystems/workflow.md)——本引擎实现的 seam 约定。
