...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-subagent-codex)是每个受支持字段及其 JSDoc 的穷尽式真源。已配置的 `model` 会原样传给每个临时 `thread/start`；省略时保留原生模型选择。提供方不会发现模型、改写别名、选择 `modelProvider` 或 `serviceTier`，也不会设置 fallback。具有凭证特征的环境变量会在显式 `env` 覆盖生效前被移除，因此供子进程使用的 API 密钥必须在该配置中显式提供。
...
- [Subagent 子系统](../../../docs/subsystems/subagent.md)——服务约定、提供方约定与终态结果语义。
...
- [生成配置目录](../../../docs/config-catalog.md#deepseek-aidsh-subagent-codex)——每个受支持配置字段及其源声明。
