...
提供方上线后 `ctx.settings` 即出现。完整配置面由提供方 README 负责；生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-settings-file)列出每个受支持字段。
...
- [设置子系统参考](../../../docs/subsystems/settings.md)——namespace、注册、owner scope、descriptor、变更提交与生成的 cordis 接口面。
- [文件型设置提供方](../settings-file/README.zh.md)——随附的 YAML/JSON 提供方：配置、热重载、保留注释的写入。
- [设置包映射](../README.zh.md)——用户设置能力的两个包及其角色。
- [能力 seam](../../../docs/capability-seams.md)——本服务遵循的 Service Definition / Service Provider / Consumer 拆分。
