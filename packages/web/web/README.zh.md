...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-web)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
两个调用都接受可选的 `AbortSignal`，用于把取消转发给提供方。规范化的请求与结果形状是调用方赖以构建的约定；[web 子系统](../../../docs/subsystems/web.md) 参考中的词汇章节对其有穷尽式描述。
...
请求与结果类型定义了调用方赖以构建的规范化词汇——`Search` 请求／结果对与 `Fetch` 请求／结果对各一组——穷尽式字段与 JSDoc 见 [`src/types.ts`](src/types.ts) 与 [web 子系统](../../../docs/subsystems/web.md) 参考。两个刻意的选择塑造了它们：`WebFetchBody` 是这里拥有的封闭联合（`html` | `text`），因此新增类型会破坏编译，直到每个消费方都处理它；`WebError` 继承 `HarnessError`，携带开放的字符串 `code`，因此消费方必须容忍提供方专有的取值。来源字段保持可选，因为并非每个提供方都返回全部字段。
...
- [web 子系统](../../../docs/subsystems/web.md)——穷尽式的搜索／抓取请求与结果、提供方可用性与错误码。
...
- [生成配置目录](../../../docs/config-catalog.md#deepseek-aidsh-web)——每个受支持配置字段及其源声明。
