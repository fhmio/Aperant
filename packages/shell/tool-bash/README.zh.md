...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-tool-bash)是每个受支持字段及其 JSDoc 的穷尽式真源；生成的[工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-tool-bash)携带完整参数 schema。
...
- [Bash 执行器子系统](../../../docs/subsystems/shell.md)——请求／spec 词汇、结果与后台进程。
...
- [生成的工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-tool-bash)——`bash` 参数 schema 的确切内容。
- [生成的配置目录](../../../docs/config-catalog.md#deepseek-aidsh-tool-bash)——每个受支持配置字段及其源声明。
...
模型会看到生成的 [`bash` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-bash)。仅当本生产方启用 `run_in_background` 时，该字段才会出现；仅当已挂载执行器声明支持沙箱时，`sandbox_permissions` 和 `justification` 才会出现。按 agent 作用域限制工具可以移除该 agent 的定义。
