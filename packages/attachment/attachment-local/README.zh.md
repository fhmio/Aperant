...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-attachment-local)完整列出了所有受支持的字段及其 JSDoc。
...
- [附件子系统参考](../../../docs/subsystems/attachment.md)——服务约定、载荷类型与 `ctx.attachments` 的 Cordis 接口面。
- [附件 seam 包](../attachment/README.zh.md)——本存储支撑的图片附件能力。
- [生成配置目录](../../../docs/config-catalog.md#deepseek-aidsh-attachment-local)——每个受支持配置字段及其源声明。
