...
该值必须是正的安全整数。它既提供模型自行报告阻塞的硬下限，也决定模型指引中指明的数值。生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-tool-goal)是每个受支持字段的穷尽式真源。
...
- [生成的工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-tool-goal)——模型接收的精确 schema。
