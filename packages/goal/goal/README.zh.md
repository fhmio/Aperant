...
`defaultMaxGoalRounds` 必须是正的安全整数；指定了自身上限的 create 请求会覆盖它。生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-goal)是每个受支持字段的穷尽式真源。
...
- [goal 子系统](../../../docs/subsystems/goal.md)——goal 类型、持久的变更载荷与生成的服务 API。
- [goal 组地图](../README.zh.md)——goal 各包及其组合方式。
- [生成的配置目录](../../../docs/config-catalog.md#deepseek-aidsh-goal)——每个受支持配置字段及其源声明。
