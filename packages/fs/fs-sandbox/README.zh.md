...
后端的配置与本地后端完全相同（`cwd` 解析默认值与 `diffBasisMaxBytes` 覆写上限）；[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-fs-sandbox)是完整配置的真源。
...
- [进程沙箱子系统](../../../docs/subsystems/sandbox.md)——模式、逐调用策略与故障关闭错误。
