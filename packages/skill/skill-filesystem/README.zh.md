...
其余 `watch*` 字段用于调节 Chokidar 行为——轮询、稳定窗口、间隔、项目上限与符号链接跟随。生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-skill-filesystem)完整列出了所有字段，是这些字段的真源。
...
- [skill 子系统参考](../../../docs/subsystems/skills.md)——注册表约定与本地发现优先级表。
