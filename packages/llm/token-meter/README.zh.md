...
- [Token 计量子系统](../../../docs/subsystems/token-meter.md)——`ctx.tokenMeter` 背后的测量语义。
- [dsh-llm 服务](../llm/README.zh.md)——其容量元数据由 `resolveModelInfo()` 提供的模型调用服务。
- [压缩能力](../../../docs/subsystems/compaction.md)——读取 `measure()` 的压力敏感消费方。
...
- [LLM 流式子系统](../../../docs/subsystems/llm-streaming.md)——本服务计价的消息与块类型。
