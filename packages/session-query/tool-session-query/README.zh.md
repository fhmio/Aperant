...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-tool-session-query)是每个受支持字段及其 JSDoc 的穷尽式真源。
...
- [生成的工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-tool-session-query)——模型看到的五个工具 schema。
...
- [会话查询子系统参考](../../../docs/subsystems/session-query.md)——工具之下的类型级约定。
...
模型会看到生成的 [`session_search`、`session_event_search`、`session_trace`、`session_event_trace` 与 `session_event_read` schema](../../../docs/tool-catalog.md#deepseek-aidsh-tool-session-query)。搜索过滤器会增加固定 schema token，而游标、工作区路径、输出分页与模型可控结果上限仍不存在。
