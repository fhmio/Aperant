...
无法成为提醒的输入——空提示词、多于一个 selector、无效时区、非未来或超出范围的时间、低于 5 分钟的重复间隔——会返回稳定的错误代码而不是成功。生成的[工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-schedule)拥有每个工具接受的精确参数。
...
- [仅限会话内的 Schedule 子系统](../../../docs/subsystems/schedule.md)——带精确类型定义的持久记录、转换、视图与交付约定。
- [生成的工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-schedule)——模型接收的 `schedule_create`、`schedule_list` 与 `schedule_delete` 完整 schema。
...
- [Schedule 用户指南](../../../docs/user/guide/schedule.md)——挂载本包与 time-context 的官方配置路径。
...
只有在此插件加载后创建的 live 根 agent 中，模型才会看到三个生成的工具 schema；[生成的工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-schedule)拥有精确的参数与结果 schema。工具结果包含上文所述的规范 JSON 值。
