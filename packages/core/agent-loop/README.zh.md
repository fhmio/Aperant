...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-agent-loop)是每个受支持字段的穷尽式真源。适配器会校验有效推理强度，循环则把它记录在请求头中。`maxParallelToolCalls` 也是整个 `agent-loop` 设置分节，因此叠加在该条目之上的用户层无需重启即可限制下一组工具调用。
...
- [Core 子系统](../../../docs/subsystems/core.md)——轮次流与拦截决策。
- [会话子系统](../../../docs/subsystems/session.md)——循环写入并据此派生的持久日志。
- [工具子系统](../../../docs/subsystems/tools.md)——循环分发所经过的流水线。
