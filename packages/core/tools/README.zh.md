...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-tools)是每个受支持字段的穷尽式真源。非原生模式要求已组合的 `ctx.codeRuntime` 且其语言有已注册的 SDK 渲染器；agent preset 通过 [`dsh-agent-tool-presentation`](../agent-tool-presentation/README.zh.md) 自行选择呈现方式，单个 agent 可用 `presentAs(mode)` 遮蔽默认值。
...
- [工具子系统](../../../docs/subsystems/tools.md)——完整流水线类型、schema DSL 与生成的服务 API。
- [生成工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-tools)——模型收到的已交付工具 schema。
- [工具执行流水线](../../../docs/tool-execution-pipeline.md)——可视化流水线。
- [添加工具实操手册](../../../docs/cookbook/adding-a-tool.md)——分步骤的工具编写指南。
...
在普通模式下，模型会看到每个可见定义的确切名称、描述与 JSON Schema；已交付定义记录在生成的[工具目录](../../../docs/tool-catalog.md#deepseek-aidsh-tools)中。agent 作用域的限制、遮蔽与扩展注册会改变该 agent 的最终工具集合。
