...
生成的[配置目录](../../../docs/config-catalog.md#deepseek-aidsh-user-approval)是每个受支持字段及其 JSDoc 的穷尽式真源。`setPolicy(agent, policy)` 切换运行中的 agent 的策略，并为它的下一个模型步骤排队一条「由用户更改」消息；`setApprovalPolicy(session, policy)` 是会话初始化使用的直接持久写入路径。
...
- [审批子系统参考](../../../docs/subsystems/approval.md)——共享的请求／结果词汇与 `ctx.approval` 的 Cordis 接口面。
