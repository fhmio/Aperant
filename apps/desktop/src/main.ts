...
function resolveAppIcon(): string | undefined {
  const ico = fileURLToPath(new URL('../resources/icon.ico', import.meta.url))
  const png = fileURLToPath(new URL('../resources/icon.png', import.meta.url))
...
function createWindow(preload: string, show = false): BrowserWindow {
  ...
  if (icon !== undefined && typeof window.setIcon === 'function') {
    window.setIcon(icon)
  }
...
async function main(): Promise<void> {
  if (process.platform === 'win32') {
    app.setAppUserModelId?.('com.deepseek.aperant')
  }
