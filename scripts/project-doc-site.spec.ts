...
describe('docsPages locale routes', () => {
  ...
  it('projects every published subsystem page in English', () => {
    ...
    expect(rootPages.every(page => page.source.endsWith('.md'))).toBe(true)
  })
  ...
  it('projects reviewed generated counterparts into root locale routes', () => {
    ...
    expect(pages.every(page => page?.contentLocale === 'en-US')).toBe(true)
    expect(pages.every(page => page?.source.endsWith('.md'))).toBe(true)
  })
})
...
describe('sidebar ordering', () => {
  ...
  it('refuses a section with no declared placement', () => {
    expect(() => sectionSpec('root', 'UnknownSection'))
      .toThrow('Sidebar section "UnknownSection" has no placement in the root locale.')
  })

  it('declares placements per locale', () => {
    ...
    expect(() => sectionSpec('en', 'UnknownSection')).toThrow()
    expect(() => sectionSpec('root', 'UnknownSection')).toThrow()
  })
...
describe('llmsTxt', () => {
  ...
  it('groups the two locale trees under their own headings', () => {
    const text = llmsTxt(site)
    expect(text.indexOf('## English (Root)')).toBeGreaterThan(-1)
    expect(text.indexOf('## English (Mirror)')).toBeGreaterThan(text.indexOf('## English (Root)'))
  })
