...
const WORKSPACE_ROOT_PACKAGES = new Set(['@deepseek-ai/dsh-root', 'aperant'])
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (WORKSPACE_ROOT_PACKAGES.has(name)) throw new Error(`${normalized} selected the workspace root`)
