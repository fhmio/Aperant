 ...
 * pairs `ar`/`en` across sibling files as well as within one module. An `ar`
...
function dictionariesIn(file: string): Dictionary[] {
  ...
  // between `r` and an uppercase letter.
 ...
 * matching the admission of the cheap pre-filter, so `ar2Foo`/`ar_probe`
